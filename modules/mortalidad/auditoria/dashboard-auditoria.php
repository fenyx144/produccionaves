...
$listarApiUrl = $baseModUrl . '/listar_auditorias.php';
$detalleApiUrl = $baseModUrl . '/get_detalle_auditoria.php';
$hoy = date('Y-m-d');
$mesActual = date('Y-m');
$anio = date('Y');
?>
...
<head>
    ...
    <title>Mortalidad — Auditoría</title>
    ...
    <style>
        ...
        .mort-aud-grupo { border: 1px solid #e5e7eb; border-radius: 10px; padding: 12px; margin-bottom: 10px; }
        ...
        .granja-optgroup-label {
            font-weight: 600; font-size: 0.8rem; color: #1e88e5;
            padding: 4px 8px; background: #eff6ff; border-radius: 4px; margin: 4px 0;
        }
        .dt-top-row {
        ...
        select.granja-select option:disabled {
            font-weight: 700; color: #1e88e5; background: #eff6ff;
        }
        .aud-foto-grid { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; }
        .aud-foto-grid img {
...
<div class="w-full max-w-full py-4 px-4 sm:px-6 lg:px-8 box-border">
    ...
    <div class="card-filtros-compacta mb-6 bg-white border rounded-2xl shadow-sm overflow-hidden">
        <button type="button" id="btnToggleFiltrosAud"
            ...
            <svg id="iconoFiltrosAud" class="w-5 h-5 text-gray-600 transition-transform duration-300"
                fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
            </svg>
        </button>
        <div id="contenidoFiltrosAud" class="px-6 pb-6 pt-4 hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-4">
                <div>
                    ...
                    <div class="selector-display">
                        <input id="mrt-aud-granja-resumen" type="text" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-sky-500 hc-sel-granja" readonly placeholder="Clic para seleccionar" value="">
                    </div>
                    <input id="mrt-aud-h-granja" type="hidden" value="">
                    <input id="mrt-aud-h-campania" type="hidden" value="">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div>
                    ...
                    <button type="button" id="btnAbrirGruposFiltro" class="btn-grupos-filtro">
                        ...
                        <i class="fas fa-chevron-right text-sky-600 text-sm"></i>
                    </button>
                </div>
            </div>
            <div class="dashboard-actions mt-6 flex flex-wrap justify-start gap-4">
                <button type="button" id="btnFiltrarAud" class="px-6 py-2.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 font-medium inline-flex items-center gap-2">
                    <i class="fas fa-search"></i> Buscar
                </button>
                <button type="button" id="btnLimpiarAud" class="px-6 py-2.5 rounded-lg border border-gray-300 text-gray-700 bg-gray-100 hover:bg-gray-200 font-medium inline-flex items-center gap-2">
    ...
    <div class="grid grid-cols-2 gap-3 mb-6">
        <div class="resumen-card"><div class="etiqueta">Registros auditados</div><div class="valor" id="resumenRegistros">0</div></div>
    ...
    <div class="tabla-listado-wrapper bg-white rounded-xl shadow-md p-5">
        <div class="table-wrapper overflow-x-auto">
            <table id="tablaAuditoria" class="data-table display w-full text-sm border-collapse config-table" style="width:100%">
                <thead>
                    <tr>
                        <th>N°</th>
                        <th>Fecha</th>
...
<div id="modalGruposFiltro" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-[60] p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div class="px-5 py-4 overflow-auto flex-1">
            <p class="text-sm text-gray-500 mb-4">Seleccione uno o más grupos. Si no marca ninguno, se muestran todos.</p>
        ...
        <div class="px-5 py-3 border-t flex justify-between gap-2">
            <button type="button" id="btnLimpiarGruposModal" class="btn-outline text-sm py-2 px-3">Quitar selección</button>
...
<?php
$gmc_id_prefix = 'mrt-aud';
$gmc_shell_panel_id = 'mrt-aud-modal-granjas';
$gmc_show_chk_todas = false;
require __DIR__ . '/../../../core/lib/modals/modal_granjas_campanias.php';
?>

<div id="modalDetalleAud" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h5 class="text-lg font-semibold text-gray-800" id="modalDetalleAudTitulo">Detalle de auditoría</h5>
            <button type="button" id="btnCerrarDetalleAud" class="text-gray-500 hover:text-gray-700 text-2xl leading-none">&times;</button>
        </div>
        <div class="px-6 py-5 overflow-auto flex-1 text-sm" id="modalDetalleAudBody"></div>
    </div>
</div>

<script>
window.MORT_AUD_LISTADO_CFG = {
    listarUrl: <?= json_encode($listarApiUrl, JSON_UNESCAPED_UNICODE) ?>,
    detalleUrl: <?= json_encode($detalleApiUrl, JSON_UNESCAPED_UNICODE) ?>,
    arbolGrupos: <?= json_encode($arbolGrupos, JSON_UNESCAPED_UNICODE) ?>,
    granjasMeta: <?php
$gm = [];
...
<script src="js/mortalidad-auditoria-listado.js?v=<?php echo (int) @filemtime(__DIR__ . '/js/mortalidad-auditoria-listado.js'); ?>"></script>
</body>
</html>
